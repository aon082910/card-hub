...
const DICTIONARY = {
  en: {
    ...
    scan_setup_link_text: 'MakerWorld card scanner stand', scan_setup_hint2: ' — point the camera at the tray or slot where each card lands. It waits for the card to stop moving, then snaps just that card (cropped to the capture zone) — no need to remove it before the next one lands on top.',
    ...
    scan_resolution: 'Camera resolution', scan_resolution_auto: 'Auto (camera default)', scan_actual_resolution: 'Actual stream resolution',
    ...
    scan_tuning_hint: "Set the zone to the tray/slot where the card comes to rest — the photo is cropped to exactly this rectangle. Capture fires once the card stops moving there, not while it's still sliding, so a fast ramp won't get blurry half-captures. Watch the meter above the video while a card slides through: it should spike above the line, then drop back below once it settles. Raise Sensitivity if it never spikes; lower it if it never settles.",
    scan_state_empty: 'Ready — slide a card in', scan_state_detecting: 'Card moving, waiting for it to settle...', scan_state_captured: 'Captured!',
  ...
  es: {
    ...
    scan_setup_link_text: 'soporte escáner de cartas de MakerWorld', scan_setup_hint2: ' — apunta la cámara a la bandeja o ranura donde cae cada carta. Espera a que la carta deje de moverse y luego captura solo esa carta (recortada a la zona de captura) — no hace falta retirarla antes de que caiga la siguiente encima.',
    ...
    scan_resolution: 'Resolución de cámara', scan_resolution_auto: 'Automática (predeterminada de la cámara)', scan_actual_resolution: 'Resolución real de la transmisión',
    ...
    scan_tuning_hint: 'Ajusta la zona a la bandeja/ranura donde queda la carta — la foto se recorta exactamente a ese rectángulo. La captura se dispara cuando la carta deja de moverse ahí, no mientras aún se desliza, así que una rampa rápida no producirá capturas borrosas a medias. Observa el medidor sobre el video mientras pasa una carta: debería subir por encima de la línea y luego bajar de nuevo cuando se asiente. Sube la Sensibilidad si nunca sube; bájala si nunca se asienta.',
    scan_state_empty: 'Listo — desliza una carta', scan_state_detecting: 'Carta en movimiento, esperando a que se asiente...', scan_state_captured: '¡Capturada!',
  ...
  fr: {
    ...
    scan_setup_link_text: 'support scanner de cartes MakerWorld', scan_setup_hint2: " — pointez la caméra vers le bac ou la fente où chaque carte atterrit. Il attend que la carte s'immobilise, puis capture uniquement cette carte (recadrée à la zone de capture) — inutile de la retirer avant que la suivante n'atterrisse dessus.",
    ...
    scan_resolution: 'Résolution de la caméra', scan_resolution_auto: 'Auto (par défaut de la caméra)', scan_actual_resolution: 'Résolution réelle du flux',
    ...
    scan_tuning_hint: "Réglez la zone sur le bac/la fente où la carte s'arrête — la photo est recadrée exactement sur ce rectangle. La capture se déclenche une fois que la carte s'y immobilise, pas pendant qu'elle glisse encore, donc une rampe rapide ne produira pas de captures floues à mi-course. Surveillez la jauge au-dessus de la vidéo pendant qu'une carte passe : elle devrait dépasser la ligne puis redescendre en dessous une fois la carte immobilisée. Augmentez la Sensibilité si elle ne dépasse jamais la ligne ; diminuez-la si la carte ne semble jamais s'immobiliser.",
    scan_state_empty: 'Prêt — faites glisser une carte', scan_state_detecting: "Carte en mouvement, en attente qu'elle s'immobilise...", scan_state_captured: 'Capturée !',
