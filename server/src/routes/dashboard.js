...
router.get('/', (req, res) => {
  ...
  const salesTotals = db.prepare(`
    SELECT
      ...
      COALESCE(SUM(fees + shipping_cost), 0) as total_costs,
  ...
  const recentSales = db.prepare(`
    ...
    JOIN cards c ON c.id = s.card_id ORDER BY s.sale_date DESC LIMIT 10
  `).all();

  const snapshots = db.prepare(`
    SELECT date(taken_at) as day, total_value, total_cost FROM portfolio_snapshots ORDER BY taken_at
  `).all();
