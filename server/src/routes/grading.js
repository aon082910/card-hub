...
function ownedSubmission(id, userId) {
  return db.prepare(`
    SELECT g.* FROM grading_submissions g JOIN cards c ON c.id = g.card_id WHERE g.id = ? AND c.user_id = ?
  `).get(id, userId);
}
...
router.post('/', (req, res) => {
  ...
  logAudit({ userId: req.session.userId, action: 'create', entityType: 'grading_submission', entityId: info.lastInsertRowid, details: `${company} - card #${card_id}` });
  res.status(201).json(db.prepare('SELECT * FROM grading_submissions WHERE id = ?').get(info.lastInsertRowid));
});

router.put('/:id', (req, res) => {
  const existing = ownedSubmission(req.params.id, req.session.userId);
...
router.delete('/:id', (req, res) => {
  if (!ownedSubmission(req.params.id, req.session.userId)) return res.status(404).json({ error: 'not found' });
